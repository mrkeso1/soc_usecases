...
class RelatedD3FendsInline(admin.TabularInline):
    """
    Permite ver/editar desde ATT&CK qué D3FEND están relacionados.
    Usa la tabla intermedia generada por D3Fend.related_attacks.
    """
    model = D3Fend.related_attacks.through
    extra = 0
    verbose_name = "D3FEND relacionado"
    verbose_name_plural = "D3FEND relacionados"
...
class MitreAttackAdmin(admin.ModelAdmin):
    ...
    def d3fend_preview(self, obj):
        d3fends = obj.related_d3fends.all().order_by("code", "name")[:5]
        ...
        for d3 in d3fends:
            label = f"{d3.code} - {d3.name}" if d3.name else d3.code
            values.append(label)

        total = obj.related_d3fends.count()

        if total > 5:
            values.append(f"... +{total - 5}")

        return " | ".join(values)

    @admin.display(description="D3FEND relacionados completos")
...
class D3FendAdmin(admin.ModelAdmin):
    ...
    @admin.display(description="ATT&CK relacionados completos")
