...
def run_scheduled_mitre_attack_sync(
    *,
    ...
    if not settings:
        return MitreAttackSyncResult(ran=False, message="No hay configuracion activa de sincronizacion MITRE.")
    ...
    if not force and not settings.is_due(now):
        ...
        if next_run:
            message = f"{message} Proxima ejecucion: {timezone.localtime(next_run):%Y-%m-%d %H:%M}."
