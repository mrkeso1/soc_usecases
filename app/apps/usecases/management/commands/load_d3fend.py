...
class Command(BaseCommand):
    help = "Carga D3FEND y sincroniza relaciones D3FEND→ATT&CK usando también el CSV de mappings como catálogo"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--mappings-only",
            action="store_true",
            help="No recarga el CSV principal; solo sincroniza mappings D3FEND→ATT&CK.",
        )
    ...
    def _mapping_d3fend_candidates(self, row: dict) -> list[dict]:
        """
        El CSV de mappings trae D3FEND como labels y URIs, no como D3-XXXX.
        Usamos esos valores como catálogo válido.
        """
        candidates = []
    ...
    def _resolve_or_create_d3fends_from_mapping_row(self, row: dict, d3_lookup: dict):
        ...
        for candidate in self._mapping_d3fend_candidates(row):
            ...
            if "name" in model_fields:
                defaults["name"] = self._fit_field("name", label or _label_from_fragment(code) or code)
